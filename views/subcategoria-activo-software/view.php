<?php

use yii\helpers\Html;
use yii\widgets\DetailView;

/* @var $this yii\web\View */
/* @var $model app\models\SubcategoriaActivoSoftware */

$this->title = $model->SubcategoriaActivoSoftwareID;
$this->params['breadcrumbs'][] = ['label' => 'Subcategoria Activo Softwares', 'url' => ['index']];
$this->params['breadcrumbs'][] = $this->title;
?>
<div class="subcategoria-activo-software-view">

    <h1><?= Html::encode($this->title) ?></h1>

    <p>
        <?= Html::a('Update', ['update', 'id' => $model->SubcategoriaActivoSoftwareID], ['class' => 'btn btn-primary']) ?>
        <?= Html::a('Delete', ['delete', 'id' => $model->SubcategoriaActivoSoftwareID], [
            'class' => 'btn btn-danger',
            'data' => [
                'confirm' => 'Are you sure you want to delete this item?',
                'method' => 'post',
            ],
        ]) ?>
    </p>

    <?= DetailView::widget([
        'model' => $model,
        'attributes' => [
            'SubcategoriaActivoSoftwareID',
            'Nombre',
        ],
    ]) ?>

</div>
