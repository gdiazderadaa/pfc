...
<div class="elemento-hardware-form">
    ...
    <?php
        if ($model->isNewRecord){
            ...
            echo Html::beginTag('div',['class' => 'form-group row']);
            echo Html::beginTag('div',['class' => 'col-md-2']);
            echo Html::label(Yii::t('app','Bulk creation'), 'lote', ['class' => 'control-label']);
            echo SwitchInput::widget(['name'=>'lote', 
                'value'=>false,
                'pluginOptions' => [
                ...
                'switchChange.bootstrapSwitch' => 'function() { 
                    ...
                    if (this.checked == false){
                        $("label[for=cantidad]").hide("slow");
                        $("#cantidad").hide("slow");
                        }
                        else {
                            $("label[for=cantidad]").show("slow");
                            $("#cantidad").show("slow");
                        }
                    }'
                ]
            ...
            echo Html::beginTag('div',['class' => 'col-md-2']);
            echo Html::label(Yii::t('app','Quantity'),'cantidad', ['class' => 'control-label']);
            echo Html::Input('text','cantidad','',['id' =>'cantidad','class' => 'form-control']);
            echo Html::endTag('div');
            echo Html::endTag('div');
            echo Html::endTag('div');
    ...
    <?= $form->field($model, 'estado')->dropDownList($model->getEstados(),['prompt'=>Yii::t('app', '- Select the status -') ]) ?>
